...
For a quick reference (the TLDR version), please see the other vignette "Vignette - summary of scFeatures functions". 
...
# Data required for this vignette

Please click on the link to download all the example data required for this vignette.  
...
For demonstration purpose, we provide the melanoma pre-treatment dataset as used in our manuscript. The dataset is taken from the study Sade-Feldman et al. (2018). Defining T cell states associated with response to checkpoint immunotherapy in melanoma. Cell, 175(4), 998-1013.

scFeatures takes in data as a Seurat object, with the gene expression stored in the assay `RNA`. The required metadata is the `celltype` and `sample` label. The `condition` label is optional, and is only required if users wish to train a classification model on disease condition using the generated features.    

Here, we provide example of how to construct a Seurat object from a count matrix and the corresponding label file. 


```{r include=FALSE, paged.print=TRUE}
setwd("/dskh/nobackup/yuec/scfeatures/scfeatures_package/scFeatures/inst/extdata")
```


```{r eval=TRUE, include=TRUE}
data_matrix <- readRDS("example_scrnaseq_matrix.rds")
data_meta <- readRDS("example_scrnaseq_label.rds")
data <- Seurat::CreateSeuratObject( data_matrix, meta.data = data_meta)

data <- process_data(data) # perform normalisation of the count 
 
# confirm the data contain cell types 
unique( data$celltype )[1:5]
...
* Detail:   
This feature category aggregate the cells across each cell type and then construct various features. For all feature types, we restrict to the top variable genes to reduce the dimensions of the feature. The argument `num_top_gene` determines how many genes to include. By default, the algorithm picks around 100 most variable genes per cell type.   
...
# we can change this number to pick ~50 genes per cell type
...
* Detail: 
The feature types in this category is calculated based on the aggregated expressions across all cells. The argument `num_top_gene` determines how many genes to include. By default, the algorithm picks 1500 most variable genes.
...
# we can change this number to pick 1000 genes 
...
## Classification using the generated features
...
For demonstration purpose, we provide the triple negative breast cancer patients dataset as used in our manuscript. The dataset is taken from the study Keren et al. (2019). MIBI-TOF: A multiplexed imaging platform relates cellular phenotypes and tissue structure. Science advances, 5(10), eaax5851.

For spatial proteomics, the required information are `sample`, `x_cord` and `y_cord` (the x- and y- coordinates of the cell). 

```{r eval=TRUE, include=TRUE}
data <-  readRDS("example_spatial_proteomics.rds")
...
For demonstration purpose, we provide the ALS dataset as used in our manuscript. The dataset is taken from the study Maniatis et al. (2019). Spatiotemporal dynamics of molecular pathology in amyotrophic lateral sclerosis. Science, 364(6435), 89-93.
...
```{r eval=TRUE, include=TRUE}
data <- readRDS("example_spatial_transcriptomics.rds")
