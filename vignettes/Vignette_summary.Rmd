...
The above function uses all default settings and generate all feature types. We provide an example below on customising each of the parameters.   
...
scfeatures_result <- scFeatures(data, 
             ...
             feature_types = c("proportion_raw", "pathway_gsva", "L_stats", "gene_mean_celltype",
                               "gene_prop_aggregated") , 
             # by default assumes `scrna`, this is now set to `spatial_p`.   
             type = "spatial_p" ,
...
Below we list the function for generating each individual feature types for scRNA-seq, spatial proteomic and spatial transcriptomic data and the input data format. 



# Converting SingleCellExperiment and SpatialExperiment object to Seurat object

scFeatures takes a Seurat data as input. We provide a function `makeSeurat()` for the conversion.   

The SingleCellExperiment or SpatialExperiment object needs to contain a column named "sample" and a column named "celltype".  
...
Alternatively, user can provide the assay to use in the `assay` argument.     


If users want to construct features from the spatial metrics category, by default we need columns called "x_cord" and "y_cord".      
Alternatively, user can provide the names of the columns containing the coordinates in the `spatialCoords` argument.     

For spot-based spatial transcriptomics, we also requires a matrix containing cell type prediction probability of each spot, in the format of celltype x spot, where each entry represents the prediction probability of each cell type for each spot.    

```{r eval=FALSE, include=TRUE}
# example not run
converted_data <- makeSeurat(my_data,  assay = "norm",  sample = "imageID", 
