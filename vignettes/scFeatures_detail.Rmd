...
scfeatures_result <- scFeatures(data,
    ...
    type = "spatial_p",
    # set species to `Mus musculus`.
    species = "Mus musculus",
    # set to user specified pathways.
    geneset = list(
        "pathway_a" = c("SMA", "Vimentin"),
        "pathway_b" = c("B7H3", "FoxP3")
    ),
    ...
    celltype_genes = data.frame(
        celltype = c("Macrophages", "Mono/Neu"),
        marker = c("CD3", "p53")
    ),
...
data <- readRDS(
    system.file("extdata",
        "example_spatial_proteomics.rds",
        package = "scFeatures"
    )
)

# data were previously normalised
...
# plot survival curve
fit <- survfit(
    Surv(Survival_days_capped_2016.1.1, Censoring) ~ cluster,
    data = meta_patients
)
...
# for spatial transcriptomics, we need to estimate the number of cells in each
# spot
...
feature_pathway_mean <- run_pathway_mean(
    data,
    geneset = NULL, species = "Mus musculus", type = "spatial_t"
)
```
