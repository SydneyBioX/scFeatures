...
The above function uses all default settings and generate all feature types. We provide an example below that describes how this can be done with a spatial proteomics dataset.   
...
feature_proportion_ratio <- run_proportion_ratio(data , ncores = 4)
```
...
feature_gene_mean_celltype <- run_gene_mean_celltype(data_remove_mito, ncores = 4)
...
feature_pathway_mean <- run_pathway_mean(data, ncores = 4)
feature_pathway_prop <- run_pathway_prop(data, ncores = 4)
```
...
```{r include=TRUE}
feature_CCI <- run_CCI(data, ncores = 4) 
```
...
```{r, include=TRUE}
feature_gene_mean_aggregated <- run_gene_mean(data, ncores = 4)
...
feature_gene_prop_aggregated <- run_gene_prop(data, num_top_gene = 100, ncores = 4)
```
...
feature_proportion_ratio <- run_proportion_ratio(data, type = "spatial_p", ncores = 4)
```
...
```{r, include=TRUE}
feature_gene_mean_celltype <- run_gene_mean_celltype(data, type = "spatial_p", ncores = 4)
feature_gene_prop_celltype <- run_gene_prop_celltype(data, type = "spatial_p", ncores = 4)
feature_gene_cor_celltype <- run_gene_cor_celltype(data, type = "spatial_p", ncores = 4)
```
...
```{r, include=TRUE}
feature_gene_mean_aggregated <- run_gene_mean(data, type = "spatial_p", ncores = 4)
feature_gene_prop_aggregated <- run_gene_prop(data, type = "spatial_p", ncores = 4)
feature_gene_cor_aggregated <- run_gene_cor(data, type = "spatial_p", ncores = 4)
```
...
```{r, include=TRUE}
feature_L_stats <- run_L_function(data, type = "spatial_p", ncores = 4)
feature_morans_I <- run_Morans_I(data, type = "spatial_p", ncores = 4)
feature_celltype_interaction <- run_celltype_interaction(
    data,
    type = "spatial_p",
    ncores = 4
)
feature_nn_correlation <- run_nn_correlation(data, type = "spatial_p", ncores = 4)
```
...
feature_proportion_ratio <- run_proportion_ratio(data, type = "spatial_t", ncores = 4)
```
...
```{r, eval=FALSE, include=TRUE }
feature_gene_mean_aggregated <- run_gene_mean(data, type = "spatial_t" , ncores = 4)
feature_gene_prop_aggregated <- run_gene_prop(data, type = "spatial_t" , ncores = 4)
feature_gene_cor_aggregated <- run_gene_cor(data, type = "spatial_t", ncores = 4)
```
...
```{r, eval=FALSE, include=TRUE}
feature_L_stats <- run_L_function(data, type = "spatial_t", ncores = 4)
feature_morans_I <- run_Morans_I(data, type = "spatial_t", ncores = 4)
feature_celltype_interaction <- run_celltype_interaction(
    data,
    type = "spatial_t",
    ncores = 4
)
feature_nn_correlation <- run_nn_correlation(data, type = "spatial_t", ncores = 4)
```
