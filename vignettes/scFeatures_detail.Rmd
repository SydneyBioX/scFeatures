...
vignette: >
  %\VignetteIndexEntry{Detailed explanation of scFeatures}
...
The above function uses all default settings and generate all feature types. We provide an example below that describes how this can be customised with a spatial proteomics dataset.   
...
data <- process_data(data, normalise = TRUE)  



# select feature types to compute.
feature_types <- c(
    "pathway_gsva", "gene_mean_celltype",  "gene_prop_aggregated"
)
...
feature_proportion_ratio <- run_proportion_ratio(data , ncores = 2)
```
...
Note that mitochondria and ribosomal genes may not be as interesting to look at. Therefore, users can choose to remove these genes using the function `remove_mito_ribo`. This step is optional.  

```{r, include=TRUE}
data_remove_mito <- remove_mito_ribo(data)

feature_gene_mean_celltype <- run_gene_mean_celltype(data_remove_mito, ncores = 2)
...
feature_pathway_mean <- run_pathway_mean(data, ncores = 2)
feature_pathway_prop <- run_pathway_prop(data, ncores = 2)
```
...
* Detail: 
  * `CCI`: This uses the `SingleCellSignalR` package to calculate the cell-cell interaction score of ligand recepor pairs.  
...
```{r include=TRUE}
feature_CCI <- run_CCI(data, ncores = 2) 
```
...
```{r, include=TRUE}
feature_gene_mean_aggregated <- run_gene_mean(data, ncores = 2)
...
feature_gene_prop_aggregated <- run_gene_prop(data, num_top_gene = 100, ncores = 2)
```
...
To build disease prediction model from the generated features use utilise [`ClassifyR`](https://www.bioconductor.org/packages/release/bioc/html/ClassifyR.html). 
...
feature_proportion_ratio <- run_proportion_ratio(data, type = "spatial_p", ncores = 2)
```
...
```{r, eval = FALSE, include=TRUE}
feature_gene_mean_celltype <- run_gene_mean_celltype(data, type = "spatial_p", ncores = 2)
feature_gene_prop_celltype <- run_gene_prop_celltype(data, type = "spatial_p", ncores = 2)
feature_gene_cor_celltype <- run_gene_cor_celltype(data, type = "spatial_p", ncores = 2)
```
...
```{r, eval = FALSE, include=TRUE}
feature_gene_mean_aggregated <- run_gene_mean(data, type = "spatial_p", ncores = 2)
feature_gene_prop_aggregated <- run_gene_prop(data, type = "spatial_p", ncores = 2)
feature_gene_cor_aggregated <- run_gene_cor(data, type = "spatial_p", ncores = 2)
```
...
```{r, eval=FALSE, include=TRUE}
feature_L_stats <- run_L_function(data, type = "spatial_p", ncores = 2)
feature_morans_I <- run_Morans_I(data, type = "spatial_p", ncores = 2)
feature_celltype_interaction <- run_celltype_interaction(
    data,
    type = "spatial_p",
    ncores = 2
)
feature_nn_correlation <- run_nn_correlation(data, type = "spatial_p", ncores = 2)
```
...
censoring <- sample(0:1, ncol(nncor), replace = TRUE)
...
The `predictions` assay is a matrix in the form of cell types x spot, which stores the cell type probability of each spot. This can be obtained by performing cell type prediction using reference data, for example, using methods available in `r BiocStyle::CRANpkg('Seurat')` (see https://satijalab.org/seurat/articles/spatial_vignette.html#integration-with-single-cell-data-1)
...
feature_proportion_ratio <- run_proportion_ratio(data, type = "spatial_t", ncores = 2)
```
...
```{r, eval=FALSE, include=TRUE }
feature_gene_mean_aggregated <- run_gene_mean(data, type = "spatial_t" , ncores = 2)
feature_gene_prop_aggregated <- run_gene_prop(data, type = "spatial_t" , ncores = 2)
feature_gene_cor_aggregated <- run_gene_cor(data, type = "spatial_t", ncores = 2)
```
...
```{r, eval=FALSE, include=TRUE}
feature_L_stats <- run_L_function(data, type = "spatial_t", ncores = 2)
feature_morans_I <- run_Morans_I(data, type = "spatial_t", ncores = 2)
feature_celltype_interaction <- run_celltype_interaction(
    data,
    type = "spatial_t",
    ncores = 2
)
feature_nn_correlation <- run_nn_correlation(data, type = "spatial_t", ncores = 2)
```
 
 
# Reference 

1. Sade-Feldman, M., Yizhak, K., Bjorgaard, S. L., Ray, J. P., de Boer, C. G., Jenkins, R. W., ... & Hacohen, N.(2018). Defining T cell states associated with response to checkpoint immunotherapy in melanoma. Cell, 175(4), 998-1013.
2. Besag, J. (1977) Discussion of Dr Ripley's paper. Journal of the Royal Statistical Society, Series B, 39, 193–195.
3. Moran, P. A. P. (1950)."Notes on Continuous Stochastic Phenomena". Biometrika.37 (1) : 17–23. doi:10.2307/2332142
