...
if ("gene_mean_aggregated" %in% names(scfeatures_result)){

  data <- scfeatures_result$gene_mean_aggregated
  de_gene_mean_aggregated <-  find_de_not_celltype_specific(data)
  de_gene_mean_aggregated    <- data.frame("gene_mean_aggregated" ,
                       de_gene_mean_aggregated[[1]][1] ,
                        "not applicable" )
  df <- rbindlist(list(df , de_gene_mean_aggregated  ), use.names=FALSE )
} 

if ("gene_cor_aggregated" %in% names(scfeatures_result)){
  data <- scfeatures_result$gene_cor_aggregated
  de_gene_cor_aggregated <-  find_de_not_celltype_specific(data)
  de_gene_cor_aggregated    <- data.frame("gene_cor_aggregated" ,
                      de_gene_cor_aggregated[[1]][1] ,
                        "not applicable" )
  df <- rbindlist(list(df ,de_gene_cor_aggregated), use.names=FALSE )
} 

if ("gene_prop_aggregated" %in% names(scfeatures_result)){
  data <- scfeatures_result$gene_prop_aggregated
  de_gene_prop_aggregated <-  find_de_not_celltype_specific(data)
  de_gene_prop_aggregated    <- data.frame("gene_prop_aggregated" ,
...
if ("gene_mean_aggregated" %in% names(scfeatures_result)){
  ...
  data <- as.matrix ( scfeatures_result$gene_mean_aggregated)
  data <- t(data)
