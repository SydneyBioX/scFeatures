...
For demonstration purpose, we provide the melanoma pre-treatment dataset [1] as used in our manuscript. 
...
setwd("/dskh/nobackup/yuec/scfeatures/scfeatures_package/scFeatures_data")
...
# example of how to construct a Seurat object from a count matrix and the corresponding label file. 
data_matrix <- readRDS("example_scrnaseq_matrix.rds")
data_meta <- readRDS("example_scrnaseq_label.rds")
data <- Seurat::CreateSeuratObject(data_matrix, meta.data = data_meta)
```
...
For demonstration purpose, we provide the triple negative breast cancer patients dataset [2] as used in our manuscript.  
...
For demonstration purpose, we provide the ALS dataset [3] as used in our manuscript.  
