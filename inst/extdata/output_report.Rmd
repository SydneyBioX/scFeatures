...
find_de_CCI <- function(data){
   ...
   patient <- unlist( lapply( strsplit( colnames(data), "_cond_"), `[`, 1) ) 
    ...
    celltype <- unlist(lapply ( strsplit( rownames(tT), "--"), `[`, 1))
    
    celltype <-  sort(table(celltype), decreasing = T) 
...
find_de_pathway_specific <- function(data){
    ...
    celltype <- unlist( lapply(strsplit(colnames(data) , "--") , `[`, 2) )
    df <- NULL
    ...
    df <- df[order(df$prop_sig_features, decreasing = T), ]
...
find_de_celltype_specific <- function(data){
    ...
    celltype <- unlist( lapply(strsplit(colnames(data) , "--") , `[`, 1) )
    df <- NULL
    for (thiscelltype in unique(celltype)){
...
```{r}
scfeatures_result <- readRDS("/dskh/nobackup/yuec/scfeatures/scfeatures_package/scFeatures/inst/extdata/scfeatures_result.rds")
```
...
de_proportion_raw <- data.frame("feature_proportion_raw" , de_proportion_raw[[1]][1],
...
de_proportion_logit  <- data.frame("feature_proportion_logit" , de_proportion_logit [[1]][1],
...
de_proportion_ratio   <- data.frame("feature_proportion_ratio" , de_proportion_ratio[[1]][1],
...
de_gene_mean_celltype    <- data.frame("feature_gene_mean_celltype " ,
...
de_gene_prop_celltype    <- data.frame("feature_gene_prop_celltype " ,
...
de_gene_cor_celltype    <- data.frame("feature_gene_cor_celltype" ,
...
de_pathway_gsva  <- data.frame("feature_pathway_gsva" ,
...
de_pathway_mean  <- data.frame("feature_pathway_mean" ,
...
de_pathway_prop  <- data.frame("feature_pathway_prop" ,
...
de_gene_mean_aggregated <-  find_de_not_celltype_specific(data)
de_gene_mean_aggregated    <- data.frame("feature_gene_mean_aggregated" ,
                     de_gene_mean_aggregated[[1]][1] ,
                      "not applicable" )
df <- rbindlist(list(df , de_gene_mean_aggregated  ), use.names=FALSE )


data <- scfeatures_result$feature_gene_cor_aggregated
de_gene_cor_aggregated <-  find_de_not_celltype_specific(data)
de_gene_cor_aggregated    <- data.frame("feature_gene_cor_aggregated" ,
                    de_gene_cor_aggregated[[1]][1] ,
                      "not applicable" )
df <- rbindlist(list(df ,de_gene_cor_aggregated), use.names=FALSE )


data <- scfeatures_result$feature_gene_prop_aggregated
de_gene_prop_aggregated <-  find_de_not_celltype_specific(data)
de_gene_prop_aggregated    <- data.frame("feature_gene_prop_aggregated" ,
                    de_gene_prop_aggregated[[1]][1] ,
                      "not applicable" )
df <- rbindlist(list(df , de_gene_prop_aggregated ), use.names=FALSE )
...
de_L_stats  <- data.frame("feature_L_stats" ,
                   de_L_stats[[1]][1] ,   
...
de_morans_I    <- data.frame("feature_morans_I" ,
...
de_celltype_interaction    <- data.frame("feature_celltype_interaction" ,
...
de_nn_correlation  <- data.frame("feature_nn_correlation" ,
...
plot_heatmap_bulk <- function(data , num_features = 20  ){
   ...
   if (length(top_gene ) == 0){
             ...
             pheatmap( thiscelltype[top_gene, , drop=FALSE]  ,   
                 ...
                 main = label , cluster_rows = F  , color=colorRampPalette(c("navy", "white", "red"))(50) )
   }else{
...
celltype <- unlist( lapply( strsplit(celltype, "--" ), `[`, 1) )
...
```{r}
rownames( gene_output  ) <- unlist( lapply ( strsplit( rownames( gene_output  ) , "--"), `[` , 2) )
...
data <- data[, -c( which( colnames(data) == "NA"))]
...
data <- as.matrix ( scfeatures_result$feature_gene_mean_aggregated)
data <- t(data)
